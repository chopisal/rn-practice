...
import { ScrollView, StyleSheet, View, Text, Image, Share } from "react-native";
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <SharedElement id={`item.${item.key}.image`} style={styles.image}>
        <Image source={{ uri: item.image }} style={{ flex: 1, resizeMode: "contain" }} />
      </SharedElement>
      ...
      >
        {colors.map((color) => {
          return <View key={color} style={[styles.switch, { backgroundColor: color }]} />;
        })}
      </AnimatableScrollView>
      <Animatable.View
        useNativeDriver
      ...
      >
        <Text style={{ ...fonts.montserratBold, fontSize: 14 }}>Get a free service</Text>
      ...
      >
        <Text style={{ ...fonts.montserratBold, fontSize: 14 }}>Save 10% and buy now!</Text>
        <AntDesign name="arrowright" color="rgba(0,0,0,0.8)" size={20} />
      </Animatable.View>
    </SafeAreaView>
  );
